...
|-----------|----------------|---------|
| **Руководители** | Какую управленческую ценность дает агентная архитектура? | [Executive overview](overview/executive-overview.md) → [PRD](overview/prd.md) → [Architecture](architecture/system-architecture.md) |
| **Корпоративные архитекторы** | Как система помогает управлять знаниями, стандартами и решениями? | [Cheat sheet](navigation/cheat-sheet.md) → [System architecture](architecture/system-architecture.md) → [Retrieval](retrieval/) |
| **Владельцы продуктов** | Какие пользовательские сценарии уже закрыты и где текущие границы системы? | [PRD](overview/prd.md) → [Open questions](audit/open-questions.md) → [Ops & risks](operations/ops-and-risks.md) |
