# 13 — Открытые вопросы для ревью MVP

Этот список помогает отделить реализованный MVP от решений, которые требуют архитектурной проработки.

## 1. Product shell

| Вопрос | Почему важен |
|--------|--------------|
| Как должна выглядеть модель workspaces? | Нужна изоляция пользователей с разными ролями, корпусов знаний и настроек. |
| Какие роли нужны в RBAC? | Руководитель проекта, корпоративный архитектор, архитектор решения, бизнес пользователь. |
| Где пользователь загружает документы? | Нужен self-service document management UI. |
| Нужен ли embed-widget для портала или EA Tool? | Влияет на каналы доступа к агенту. |
